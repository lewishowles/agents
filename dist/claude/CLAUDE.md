...
- `project-checks` is installed globally with `uv tool install`. Use `project-checks-change-impact` for change-impact checks, `project-checks-generated-file-guard` for generated-file boundaries, `project-checks-markdown-claims` for Markdown claims, and `project-checks-repo-context` for compact repository context. Existing repo-local shims and consuming-repository `.agent/scripts/*` symlinks continue to work, so do not add a second local implementation.
