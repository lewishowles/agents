...
4. **Path-scoped rule vs. description** — skills load from their description by default. Only when a skill must apply to every file of a type, add a short Claude rule under `src/adapters/claude/rules/` with `paths:` front matter that points to the skill, then copy it in `scripts/sync.sh` and check it in `scripts/validate/check-hook-sync.sh`, as is done for `code-style.md`. That rule loads for every matching file, not just when the task mentions it.
