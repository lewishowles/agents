...
**First step, use agent-run.** Preview likely commands with `agent-run detect --json`, register a missing command during verification with `agent-run detect --add <name>` (repeatable) or `agent-run detect --all`, and report what you registered. Do not register commands during planning or review. Run the chosen command with `agent-run run <name> --json`. This is the default way to get signal, rather than raw CLI commands (`npx vitest`, `npx playwright test`, etc.). For a registration with the `file-list` capability, prefer `agent-run run <name> --file <path>` or `agent-run run <name> --glob '<pattern>'`; otherwise use the one-off `agent-run run --json -- <argv>`. Prefer scoped commands: single test file, lint on touched path, minimal repro. Ask the user for full suites, builds, e2e, or manual-only commands.
