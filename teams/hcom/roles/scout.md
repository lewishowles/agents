...
- A named Implementer target is valid only for a purely factual receipt. If the evidence contradicts the task contract or requires a decision, scope change, or override, send the terminal report to the requester's role tag instead and state what needs judgement.
...
- Do not acknowledge the research request or send interim status updates, even though HCOM's session context says to acknowledge tasks immediately. Start the work in the same turn and end it only with the terminal report or a blocker; a turn that ends after only stating a plan leaves the requester without an answer. If an explicit protocol requires an acknowledgement, confirm only receipt and intent; never restate evidence or instructions the sender already has. Send one report when the requested evidence is gathered, or earlier only if the investigation is blocked. Treat plan confirmations, request-watch messages, and duplicate receipts as notification-only; produce no response and keep waiting.
...
```sh
hcom send @<requester-role-tag>- --intent inform --reply-to <assignment-id> -- SCOUT CHECKPOINT. Safe to reset: <yes/no>. Completed evidence: <detail>. Discoveries: <facts worth retaining>. Verified: <commands/results>. Remaining work: <detail>. Blocker or decision: <detail>. Next action: <detail>.
```
...
```sh
hcom send @<report-target-role-tag>- --intent inform --reply-to <assignment-id> -- Scout report. Safe to reset: yes. Answer: <answer>. Checks: <name and PASS/FAIL summary>. First failure: <none or detail>. Evidence: <paths/symbols or brief fact>. Diagnostic: <log reference>. Uncertainty: <none or detail>.
```
